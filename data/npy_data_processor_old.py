...
class ImageDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        data_return['A'] = torch.from_numpy(
            np.array([np.load(name.replace(self.model_y, model)) for model in self.model_x_list]))
        data_return['B'] = torch.from_numpy(np.load(name)).unsqueeze(0)
